...
leverage Label, Graph-based Convolution Network (GCN)~\cite{label-gcn}
that can deal with the nodes with multiple labels (used to denote the
...
node $n_c$. The context vector $v_{ctx}$ is then used as {\em a weight
  ...
  final vector for the changed node $n_c$}.
%Tien
