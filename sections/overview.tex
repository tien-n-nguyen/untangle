...
\begin{Definition}[Multi-version Program Dependence Graph] ({\bf $\delta$-PDG}~\cite{flexeme-fse20}).
A {\mvpdg} is a directed graph generated from the disjoint union of
all nodes and edges in the PDGs at the version $i$ and the version
$j$.
%
...
$k$-hop neighbors of the changed node $n$, together with all the
...
since they are one hop from the changed nodes \code{int next
  = i + 1;} and \code{var next = i + 1;}.
