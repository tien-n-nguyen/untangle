...
of the changed statements, thus, we have the labels (concern 1,
...
clustering learning model to produce the labels for the changed
nodes/statements. For the given {\mvpdg}, the number of clusters is
unknown to the model, and it decides that number via its agglomerative
clustering algorithm. Let us present the details of the two tasks in
our CC clustering learning model.
...
\begin{figure*}[t]
	\centering \includegraphics[width=6.5in]{figures/STEP_2.png}
	\vspace{-6pt}
...
embeddings for each node in the graph. Because {\mvpdg} contains the
labels (i.e., concerns) for the nodes including changed nodes, to
better learn the useful information from the node features with the
labels, {\tool} uses the Label-GCN~\cite{} model for this.
