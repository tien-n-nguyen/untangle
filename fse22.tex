...
different purposes, both hampering program comprehension and
reduce separation of concerns. Automated approaches~have been
proposed to untangle commits with deterministic solutions.
%Unlike the state-of-the-art, deterministic untangling approaches,

In this work, we present {\tool}, a machine learning (ML)-based
...
%
We adapt the agglomerative clustering algorithm into a supervised
learning clustering model operating on the learned code change
embeddings with trainable parameters and a loss function in comparing
the predicted clusters and the correct ones during training.
%
Another key idea of {\tool} is a {\em context-aware, graph-based, code
...
for code changes}, that integrates program dependencies and
the surrounding contexts of the changes. The contexts of the changes
and cloned code are explicitly represented, helping {\tool}
...
accuracy of XX.X\%--XX.X\% relatively higher than the state-of-the-art
approaches in correctly clustering the changed statements. We also
evaluated {\tool} in a Java dataset with XX commits. The result shows
that it achieves XX.X\% relatively higher accuracy than the
