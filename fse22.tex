...
\title[Untangling Commits with Context-aware, Graph-based, Code Change Clustering Learning Model]{Untangling Commits with Context-aware, Graph-based,\\ Code Change Clustering Learning Model}
...
different concerns/purposes, both hampering program comprehension and
reduce separation of concerns. Unlike the state-of-the-art,
deterministic untangling approaches, we present {\tool}, a machine
learning (ML)-based approach that learns to untangle the changes in a
commit.
%
...
%
We adapt the agglomerative clustering algorithm into a machine
...
during training.
%
Another key of {\tool} is a {\em context-aware, graph-based, code
change representation learning (RL) model}, leveraging Graph-based
Convolution Network to produce the {\em contextualized embeddings
(vectors)} for the code changes, that integrates program dependencies,
and the surrounding contexts of the changes. The contexts of the
changes and cloned code are explicitly represented, helping {\tool}
distinguish their concerns/purposes.
%
\textcolor{red}{Our empirical evaluation on a real-world C\# dataset with 21k commits and 1,612 concerns shows that {\tool} achieves the accuracy of XX.X\%--XX\% and YY.Y\%--YY.Y\% relatively higher than the state-of-the-art approaches in cross-project and within-project settings. We also evaluated {\tool} in a Java dataset with XX commits and X,XXX concerns. The results show that {\tool} achievers higher accuracy XX.X\% relatively than the state-of-the-art approach.}
\end{abstract}
